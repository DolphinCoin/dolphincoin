...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 10000;	// stake age of full weight: 10000d
...
const string strMessageMagic = "DolphinCoin Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
    ...
    if(nHeight == 1)
    {
        nSubsidy = 10000000000 * COIN;	// 10 billion coins, that all pow coins
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    // This rule was originally applied all blocks whose timestamp was after March 15, 2012, 0:00 DOLP.
    // Now that the whole chain is irreversibly beyond that time it is applied to all blocks except the
    // two in the chain that violate it. This prevents exploiting the issue against nodes in their
    // initial block download.
    bool fEnforceBIP30 = true; // Always active in DolphinCoin
    bool fStrictPayToScriptHash = true; // Always active in DolphinCoin
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DolphinCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp =  "DOLPHINCOIN TO DA MOON";
        CTransaction txNew;
        ...
        assert(block.hashMerkleRoot == uint256("0x78bd377122f8655d7abd9728aec058970ff0b14a8f15a5601ad60d7768483234"));
